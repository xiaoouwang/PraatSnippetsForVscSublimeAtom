# this scripts saves each non empty interval to a wav file with a rectangular window. The filename corresponds to an index + interval name to avoir name overlapping.
...
form Save intervals to small sound files
	...
	sentence Folder /Users/rosingle/OneDrive/sounds/temp/
...
for interval from start_from to end_at
	...
	check = 0
	if xxx$ = "silent" and exclude_intervals_labeled_as_xxx = 1
		check = 1
	endif
...
for interval from start_from to end_at
	...
	check = 0
	if intname$ = "silent" and exclude_intervals_labeled_as_xxx = 1
		check = 1
	endif
	...
	if check = 0
		...
		intervalfile$ = "'folder$'" + "'prefix$'" + "'intnumber'" + "'suffix$'" + "'intname$'" + ".wav"
